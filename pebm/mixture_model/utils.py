...
##- Christopher Parker: get_prob_mat() returns matrix of p(x|~E) & p(x|E)
def get_prob_mat(X, mixture_models):
    ...
    if np.isnan(X).any():
        print('WARNING!!! (utils.get_prob_mat()): X has missing values! probability will be set to 1')
    for i in range(n_biomarkers):
        ...
        prob_mat[nan_mask, i, 0] = controls_pdf.squeeze()
        prob_mat[~nan_mask, i, 0] = 1 # np.mean(controls_pdf)
        prob_mat[nan_mask, i, 1] = patholog_pdf.squeeze()
        prob_mat[~nan_mask, i, 1] = 1 # np.mean(patholog_pdf)
    return prob_mat
