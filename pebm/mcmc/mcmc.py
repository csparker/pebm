...
# Author of greedy_ascent_creation_pebm(), mcmc_pebm(): Christopher S. Parker (christopher.parker@ucl.ac.uk)
...
from ..event_order_pebm import EventOrder_pebm
...
def greedy_ascent_creation_pebm(prob_mat, n_iter=1000, n_init=10, init_order=None):
    n_biomarkers = prob_mat.shape[1]
    starts_dict = dict((x, []) for x in range(n_init))
    for start_idx in range(n_init):
        current_order = EventOrder_pebm(n_biomarkers=n_biomarkers, ordering=init_order)
...
def mcmc_pebm(X, mixture_models, n_iter=10000, greedy_n_iter=1000,
         greedy_n_init=10, plot=True):
    prob_mat = get_prob_mat(X, mixture_models)
    greedy_dict = greedy_ascent_creation_pebm(prob_mat,
                                         greedy_n_iter,
                                         greedy_n_init)
    if plot:
...
def greedy_ascent_creation(prob_mat, X, mixture_models, n_iter=1000, n_init=10, initial_greedy=[], preserve_inds=[]):
    n_biomarkers = prob_mat.shape[1]
    starts_dict = dict((x, []) for x in range(n_init))
    for start_idx in range(n_init):
        current_order = EventOrder(n_biomarkers=n_biomarkers)
        if initial_greedy: current_order.ordering = initial_greedy
        current_order.score_ordering(prob_mat, X, mixture_models)
        starts_dict[start_idx].append(current_order)
        pbar = tqdm(total=n_iter)
        pbar.update(1)
        for iter_n in range(1, n_iter):
            # new_order = current_order.swap_events() if not preserve_inds else current_order.swap_events(preserve_inds)
            new_order = current_order.swap_events(preserve_inds=preserve_inds)
            new_order.score_ordering(prob_mat)
            if new_order > current_order:
                current_order = new_order
            starts_dict[start_idx].append(current_order)
...
def mcmc(X, mixture_models, n_iter=10000, greedy_n_iter=1000,
         greedy_n_init=10, plot=True, initial_greedy=[], preserve_inds=[]):
    # initial_greedy: initial sequence (optional)
    # preserve_inds: preserve ordering of these biomarker labels within the sequence (optional)
    prob_mat = get_prob_mat(X, mixture_models)
