...
class EventOrder(object):
    ...
    def score_ordering(self, prob_mat, X=[], mms=[]):
    ...
    def swap_events(self, preserve_inds=[]):
        event_order = self.ordering
        new_event_order = event_order.copy()
        if not preserve_inds:
            swap_bm = np.random.choice(event_order.shape[0], 2, replace=False)
            new_event_order[swap_bm] = new_event_order[swap_bm[::-1]]
        else:
            moveable_seq_inds = [i for i in range(len(event_order)) if event_order[i] not in preserve_inds]
            move_seq_ind = np.random.choice(moveable_seq_inds)
            seq_deleted = np.delete(arr=event_order, obj=move_seq_ind)
            seq_deleted_insert_inds_options = [i for i in range(len(seq_deleted)+1) if i != move_seq_ind]
            seq_ind_insert = np.random.choice(seq_deleted_insert_inds_options)
            # seq_ind_insert = np.random.choice(len(seq_deleted)+1)
            ind_move = event_order[move_seq_ind]
            new_event_order = np.insert(arr=seq_deleted, obj=seq_ind_insert, values=ind_move)
            print(new_event_order)
